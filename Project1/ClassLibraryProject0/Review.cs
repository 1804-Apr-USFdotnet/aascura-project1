...
namespace ClassLibraryProject0
{
    ...
    public class Review
    {
        ...
        public Review(string name, string comment, decimal rating, DateTime dateTime, int id, Restaurant restaurant)
        {
            this.id = id;
            this.name = String.Copy(name);
            this.comment = String.Copy(comment);
        ...
        public Review Copy()
        {
            Review toReturn = new Review(String.Copy(name), String.Copy(comment), rating, dateTime, id, restaurant);
            return toReturn;
        }
    }
}
